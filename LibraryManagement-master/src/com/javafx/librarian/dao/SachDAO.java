...
public class SachDAO {
    ...
    public List<Sach> getAllSach() {
        ...
        try (Connection conn = JDBCConnection.getJDBCConnection()) {
            ...
            if(Account.currentUser.getIdper() == 1)
                query = "select * from tbsach where record_status = 1";
            else
                query = "select * from tbsach where record_status = 1";
    ...
    public int editSach(Sach sach) {
        ...
        if(sach.getTinhTrang() == "Trống")
            ...
            tinhTrang = 3;
        try (Connection conn = JDBCConnection.getJDBCConnection();) {
            PreparedStatement ps = conn.prepareStatement("update tbsach set tensach=?, matheloai=?, matacgia=?, namxb=?, nxb=?, ngaynhap=?, trigia=?, tinhtrang=?, anhbia=?, soluong=? where masach=?");
